...
Next.js 15 (App Router) · TypeScript · Tailwind CSS · Neon Postgres · framer-motion · Vercel 배포
...
# 2. Neon 프로젝트 만들기 (https://console.neon.tech)
#    → "Create project" 클릭, 리전은 가까운 곳 (Asia Pacific 추천)
#    → 만들어지면 "Connection string" 복사 (postgresql://...)

# 3. 환경변수
cp .env.example .env.local
# .env.local 안에 DATABASE_URL, SERVER_SECRET, ADMIN_PASSWORD, SITE_LAUNCH_DATE 입력

# 4. 스키마 적용 — Neon Dashboard > SQL Editor 에서 순서대로 실행
#   - db/schema.sql
#   - db/seeds.sql
...
Neon Postgres
  ...
  └ RPC:    claim_slot · report_entry · ensure_day
            (@neondatabase/serverless HTTP 클라이언트로 단일 SQL 호출)
```
...
src/
  ...
  lib/
    db/client.ts                       # Neon serverless 클라이언트
